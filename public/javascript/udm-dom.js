...
if (um.wie) {
	um.eva = [];
	um.ex = [ 'onmouseover', 'onmouseout', 'onmousedown', 'onmouseup',
			'onclick', 'onmousewheel', 'onfilterchange', 'onkeydown',
			'onfocus', 'onactivate', 'onscroll', 'over', 'out' ];
	um.gg = um.ex.length;
	window.attachEvent('onunload', function() {
		...
		i = 0;
		do {
			um.gc(um.lil[i]).detachEvent((um.wie55) ? 'onactivate' : 'onfocus',
					um.eva[i]);
			i++;
		} while (i < um.lin);
...
function umIni(g) {
	...
	if (umTree && um.d) {
		...
		for (i in um.menuCode) {
			...
			if (l) {
				if (um.mie) {
					um.menuCode[i] = um.menuCode[i].replace(/<\/(li|ul)>/ig,
							'</$1>\n');
				}
...
function umNav(umTree, g) {
	...
	um.mf = 0;
	um.lf = 0;
	um.ety = typeof document.addEventListener != um.un ? 'addEventListener'
			: typeof document.attachEvent != um.un ? 'attachEvent' : '';
	...
	if (!um.ie) {
		um.tr.contains = function(n) {
			return (n == null) ? false : (n == this) ? true : this
					.contains(n.parentNode);
		};
	}
	um.lw = um.getWindowDimensions();
	um.lh = um.gc(um.tr).offsetHeight;
	if (um.og && um.hstrip[0] != 'none') {
		um.tr.style.height = (um.hstrip[1] == 'yes') ? (um.lh + um.e[17]) + 'px'
				: um.lh + 'px';
	}
...
umNav.prototype.it = function(l) {
	...
	if (umM && !um.nr) {
		if (((s && um.e[45] != 'none') || (!s && um.e[89] != 'none'))
				&& um.n.cck()) {
			if (s) {
				var r = um.e[45];
				var x = (um.ni) ? um.e[48] : '¦¦';
...
umNav.prototype.cck = function() {
	if (typeof document.defaultView != um.un
			&& typeof document.defaultView.getComputedStyle != um.un) {
		um.sa = document.defaultView.getComputedStyle(um.fl, '')
...
umNav.prototype.lr = function(l, v) {
	if (l && typeof l.style != um.un) {
		...
		if (v) {
			...
			(um.cl == '') ? l.className = 'udmR' : l.className += (l.className
					.indexOf('udmR') == -1) ? ' udmR' : '';
		} else {
...
umNav.prototype.ou = function(m, l) {
	...
	if (um.ep && um.s && m.offsetLeft < -1000) {
		var fs = um.pi(document.defaultView.getComputedStyle(m, '')
...
umNav.prototype.wpo = function(a, l, p, b, n) {
	...
	if ((n && um.ni) || (!n && um.mi)) {
		...
		a.style.clip = (um.a || um.rtl) ? 'rect(0,' + c + 'px,' + t[1]
				+ 'px,0)' : 'rect(0,' + t[0] + 'px,' + t[1] + 'px,'
				+ (t[0] - c) + 'px)';
	}
	um.xv(a);
	return true;
...
umNav.prototype.wv = function(l, n) {
	...
	if (a) {
		...
		t.src = um.baseSRC
				+ ((n) ? (r) ? um.e[45] : um.e[46]
						: (typeof um.w[p] != um.un) ? (r) ? um.w[p][23]
								: um.w[p][24] : (r) ? um.e[89] : um.e[90]);
	}
	return a;
};
umNav.prototype.pu = function(m) {
	...
	var y = (um.p) ? 2000 : 0;
	if (!((um.h || um.p) && s)) {
		x = (s) ? (um.a ? (0 - p.mw) : p.pw) : ((um.a ? (0 - p.mw) : p.pw)
				- um.e[51] - um.e[55]);
		y = (0 - p.th);
	} else if (um.h && s && um.a) {
		x = (0 - p.mw + p.tw);
	}
	x += (s) ? (um.a ? (0 - um.e[14]) : um.e[14]) : (um.a ? (0 - um.e[49])
			: um.e[49]);
...
umNav.prototype.ru = function(m) {
	...
	if (t[0] > t[1]) {
		if (s) {
			...
			p.nx = (((um.p) ? (0 - p.w - p.pw + um.e[55] - um.e[49]) : (0 - p.w
					- um.e[55] - um.e[51])) - um.e[49]);
		}
	}
	if (p.x < 0) {
		if (!s) {
	...
	t = m;
	var y = (um.wie50 && !um.p) ? ((um.pi(m.style.marginTop)
			+ m.parentNode.offsetHeight + um.getRealPosition(m.parentNode, 'y')) - p.sc)
			: (um.getRealPosition(t, 'y') - p.sc);
	while (!um.ne(t.parentNode)) {
		y += um.e[51];
		t = t.parentNode.parentNode;
	}
	...
	if (x < 0) {
		m.style.marginLeft = (um.p && um.ne(m.parentNode)) ? '2000px'
				: (p.scx > 0 ? 0 - x : 0) + 'px';
		m.style.left = '0';
	}
	return true;
...
umNav.prototype.aw = function() {
	if (um.o7 || um.mie || um.q) {
		um.rw += (um.gp(um.gc(um.tr)).offsetLeft + um.getRealPosition(um.tr,
				'x'));
	}
	if (um.mie || um.og) {
		um.rw *= 1.05;
...
umNav.prototype.ws = function() {
	...
	var w = um.getWindowDimensions();
	if ((h != um.lh && um.reset[2] != 'no')
			|| ((w.x != um.lw.x || w.y != um.lw.y) && um.reset[1] != 'no')) {
		um.closeAllMenus();
		if (um.rg) {
			um.rw = 0;
...
um.gt = function(r, t, a) {
	...
	for ( var i = 0; i < r.childNodes.length; i++) {
		if (r.childNodes[i].nodeName.toUpperCase() == t.toUpperCase()
				|| t == '*') {
...
um.getScrollAmount = function(d) {
	return ((typeof d == um.un || !d) ? (typeof window.pageYOffset != um.un ? window.pageYOffset
			: um.q ? um.m.body.scrollTop : um.m.documentElement.scrollTop)
			: (typeof window.pageXOffset != um.un ? window.pageXOffset
					: um.q ? um.m.body.scrollLeft
							: um.m.documentElement.scrollLeft));
};
...
if (typeof um.trigger != um.un && um.trigger != '' && !um.mie) {
	...
	um.drw = function() {
		...
		if (typeof um.m.getElementsByTagName != um.un
				&& um.m.getElementsByTagName('body')[0] && um.gd('udm')
				&& um.gd(um.trigger)) {
			try {
				umIni();
			} catch (err) {
