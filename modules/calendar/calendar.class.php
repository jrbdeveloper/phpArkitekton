...
class Calendar extends Base
{
	...
	public function __construct()
	{
		...
		$this->Interface	= MODULES.'calendar/template/calendar.html';
		$this->Document		= MODULES.'calendar/data/calendar.xml';
		$this->Element 		= 'event';
	}
	...
	/**
	 * This function reads the contents of the calendar xml file and presents it to the interface
	 *
	 * @access Public
	 * @param Integer $count
	 * @return String
	...
	public function Load($count=null)
	{
		...
		{
			// We only want to display a certain number of records
			if($counter < $count)
			{
	...
	private function PopulateTemplate($template, $counter)
	{
		...
		$interface = str_replace('{time}',$this->time,$interface);
		$interface = str_replace('{location}',$this->location,$interface);
	...
	private function SetPropertiesFromData($elem)
	{
		...
		$this->time 		= $elem->getElementsByTagName('time')->item(0)->nodeValue;
		$this->location 	= $elem->getElementsByTagName('location')->item(0)->nodeValue;
